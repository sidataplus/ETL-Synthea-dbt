...
ctePreDrugTarget(
    ...
    FROM {{ ref('drug_exposure') }} AS d
        JOIN {{ source('cdm', 'concept_ancestor') }} AS ca ON ca.descendant_concept_id = d.drug_concept_id
        JOIN {{ source('cdm', 'concept') }} AS c ON ca.ancestor_concept_id = c.concept_id
